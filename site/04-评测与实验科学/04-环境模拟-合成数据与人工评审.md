...
一条 Recorded Trial 可以稳定复现“模型选错政策”，却无法单独回答另一类问题：用户在第三轮补充证据后，Agent 是否会改变判断？一次查询 Timeout 后，迟到的旧政策结果是否会覆盖当前状态？虚拟时钟越过政策生效边界时，结论是否随权威事实变化？完成后续行动控制后，同一环境还要继续验证 Approval 过期和退款 Commit 后 ACK 丢失。

这些问题的答案不只存在于模型输出，还存在于用户、Tool、时间和外部世界如何反应。因此，完整的 Agent Eval 不是“对一段文本打分”，而是让候选系统在可重置、可观察、可控制的环境中完成任务，再从权威状态判定结果。
...
学完模块 07 与 09 后，再在同一 Contract 下加入 Mock Payment Ledger，分别表达“未提交”、“已 Commit”和“ACK 丢失”。
...
每个 Action 都可能改变下一轮 Observation。如果 Tool 总是返回同一段 JSON，用户总是按预写脚本回复，时钟永远不前进，评测出来的只是对这些人工假设的适应程度。
...
4. **故障可控**：Timeout、ACK 丢失、延迟 Event 和权限变更来自显式 Fault Schedule，不是运气。
...
Resolution Desk 的 Recorded Policy Result 是 Stub 式 Fixture；检查 `commit_refund` 只调用一次是 Mock 式断言；能区分“退款已 Commit 但客户端未收到 ACK”，并在重试时返回同一 Receipt，才是 Mock Payment Simulator 的领域语义。
...
本章当下只实现前两类只读 Tool；`propose_refund` 与 `commit_refund` 用来说明未来 Contract 必须保持的语义，等模块 07 建立行动控制后再接入。
...
Application Server、Simulator 和 Grader 必须使用同一个受控 Clock Port。某一层偷读系统时间，就会使边界案例产生偶发失败。
...
故障计划可以在 Commit 之前拒绝请求，也可以在 Commit 之后丢弃 ACK。两者在客户端都可能表现为 Timeout，但权威 Outcome 完全不同。重试相同 Intent 时，模拟器必须按稳定 Idempotency Key 返回既有 Receipt，而不是创建第二笔退款。

## 5. 状态化多轮 Eval

多轮 Eval 不是将几段对话串联起来，而是让每一轮都依赖当前权威状态。下图展示 Resolution Desk 最终环境的完整状态空间；当前只读实践走到 `Ready`、`Proposed` 或 `Escalated`，Approval 与外部效果分支留待模块 07 与 09 后回访：
...
2. 用约束或模型生成 Candidate，保留生成方法和 Seed；
...
当一个指标变成优化目标，系统会倾向寻找得分的最短路径。奖励投机（Reward Gaming）指系统提高了形式上的 Reward，却没有完成真实目标；评分器投机（Grader Gaming）是其中直接针对判定漏洞的情形。

Resolution Desk 中的典型例子包括：

- Grader 只查最终文本是否有“已退款”，Agent 直接声明成功；
- Tool Mock 只计数调用，Agent 传入无效金额也能得分；
...
LLM Judge 可以学习人工 Rubric 并处理大量低风险样本，但只有在独立人工集上报告一致性、False Positive 和 False Negative 后，才能进入持续评测。
