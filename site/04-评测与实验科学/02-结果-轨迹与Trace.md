...
Agent Eval 需要同时检查业务结果（Outcome）和执行轨迹（Trajectory），再由追踪记录（Trace）保存两者之间可以验证的执行线索。

## 贯穿项目：Resolution Desk

本章为 Resolution Desk 定义 Trace Fixture Contract：每条记录至少关联 `task_id`、`trial_id`、`run_id`，并包含组件、输入与输出摘要、状态变化、时间、版本和 Outcome Evidence。当前先使用 Recorded Trace 表达尚未实现的模型、Policy、Tool 与 Runtime 边界；后续组件必须产生兼容的记录，评测口径不应随实现反复改写。
...
| D     | 未退款     | Tool 已提交后确认响应（Acknowledgement，ACK）丢失，系统未对账 | Outcome 失败且恢复策略不完整       |
...
`tool.execute` 发出请求后发生网络超时，但支付系统已经创建退款。若 Trace 只记录“Tool Error”，Runtime 可能直接重试；若同时记录 Idempotency Key、Request ID 和 Outcome Reconcile，就能先查询真实结果，再安全确定后续状态。

### 场景 D：UI 显示完成但 Run 仍在工作

Provider Response 的完成事件被错误地映射为 `run.completed`，后台随后仍继续调用 Tool。问题位于 Event Adapter 或 State Machine，而不是模型输出。
