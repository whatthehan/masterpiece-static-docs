...
退款助手在 20 次运行中成功了 18 次。这个数字看起来直观，却不足以判断系统是否可靠：20 次是否来自 20 个不同 Task？两次失败是否都发生在跨租户请求？成功是由支付系统状态确认，还是由模型自己声明？换一个模型后得到 19 次成功，是否真的改善？
...
1. 为 6 个不同 Slice 的 Task 各运行一次，确认 Fixture 和确定性 Grader 正确。
2. 对其中 3 个 Task 各运行 20 次，观察 Task 内波动。
3. 使用统计库计算成功率区间，并解释样本量限制。
4. 比较 A/B 两个版本，保存逐 Task 配对结果。
5. 选择一个开放质量维度，引入 LLM Judge，并用人工样本测试顺序与冗长偏差。

验收报告必须区分：已有证据支持的结论、样本不足的趋势、以及需要生产监控才能回答的问题。
