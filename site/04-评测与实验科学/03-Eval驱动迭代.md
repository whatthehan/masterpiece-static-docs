...
Resolution Desk 的事故记录（Recorded Incident）显示：“系统引用了已经失效的退款政策。”可选改动很多：更换模型、修改 Prompt、提高 Retrieval 的 `top-k`、调整 Reranker、补充 Tool Description，或者为这个案例加入特例。如果没有先定位故障层，任何改动都可能只对当前输入偶然奏效，同时在其他场景引入回归。

Eval 驱动迭代将一次模糊反馈转换成可重复实验：保存任务与环境，定位责任层，提出单一假设，比较多个 Trial，再通过受保护指标和灰度结果决定是否发布。

## 贯穿项目：Resolution Desk

本章把前两章的静态 Dataset、Context Pack、Retrieval Result 和 Trace 组合成第一个完整的评测闭环（Eval Loop）。练习对象是事故样例 `stale_policy_003`：旧政策因措辞更相似，被排在当前政策之前。当前只验证根因假设和 Grader；尚未实现的检索器、Runtime、Shadow 与 Canary 将在后续具备执行条件时验证。
...
本章先假设工程师根据证据提出变更。系统进一步使用生产 Trace、人工修订或 Optimizer 时，不能把 Signal 直接变成在线 Prompt 或代码修改。完整链路应是 `Signal → Finding → Eval Target → Change Proposal → Candidate Behavior Bundle → Independent Eval → Approval → Release / Rollback`；其中 Finding 需要经过隐私处理、去噪和领域裁决，候选仍然接受本章的单变量实验、Protected Slice 与 Holdout 约束。完整的权限与发布设计见[受控改进：从生产证据到候选 Behavior Bundle](/masterpiece-static-docs/09-可靠性与可观测/07-受控改进-从生产证据到候选Behavior-Bundle.md)。
...
传统前端缺陷通常可以通过输入和代码版本复现；复现 Agent 缺陷还需要保存运行环境与采样配置：
...
最小复现不是保存一张聊天截图，而是构造一个可以从干净状态反复运行的 Task。若故障只在特定外部状态下出现，该状态也必须进入 Fixture。
...
使用 Resolution Desk 的事故样例 `stale_policy_003`，不要求已经存在可运行的 Agent：政策 `pol_2025_11` 已失效但相似度较高，政策 `pol_2026_07` 在任务发生时有效；Recorded Retrieval Result 将旧政策排在第一位，Recorded Trace 则显示错误发生在 Rerank 之后、Context Packing 之前。
...
6. 比较 Baseline 与候选版本的 Recorded Result；有条件时再用模型控制台补充多 Trial，但不执行 Tool。
...
验收标准：第三方能够使用同一 Fixture 重现原故障，确认它属于 Freshness 或 Ranking 层，并判断候选过滤规则是否满足接入条件。[检索、RAG 与重排](/masterpiece-static-docs/06-上下文-知识与记忆/03-检索-RAG与重排.md)实现检索管线时，必须使用这组 Regression Fixture 验证真实代码。
...
- 用户修订、模型 Reflection 或高分候选可以不经裁决直接成为系统更新。
