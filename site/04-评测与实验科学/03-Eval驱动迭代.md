...
团队收到一条生产反馈：“退款助手引用了错误政策。”可选改动很多：换模型、改 Prompt、增加 Retrieval top-k、调整 Reranker、扩写 Tool Description，或者为这个案例加入特例。如果没有先定位故障层，任何改动都可能偶然修好当前输入，同时在其他场景引入回归。
...
从现有 Agent 或 Coding Agent 任务中选择一次真实故障：

1. 保存最小 Task 和干净 Fixture。
2. 使用 Trace 将故障分类到一个主要层次。
3. 写出一个可证伪假设。
4. 将 Exact、Variant 和 Counterexample 加入 Regression Dataset。
5. 只修改一个主要变量。
6. 对 baseline 与候选进行多 Trial 配对比较。
7. 检查 Safety、Latency、Cost 和受保护 Slice。
8. 形成 `keep / revise / revert` 决策及其证据。

验收标准：第三方能够使用同一 Fixture 重现原故障，并从报告判断为什么保留或回退这次修改。
...
[下一部分：TypeScript + Node 运行时先修](/masterpiece-static-docs/05-模型接口与Agent内核/01-TypeScript-Node运行时先修.md)
