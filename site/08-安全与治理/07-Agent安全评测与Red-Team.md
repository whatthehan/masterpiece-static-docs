...
Resolution Desk 已经具有来源标记、最小权限、不可变 Proposal、AG-UI Adapter、受控 A2UI Surface 和原生审批界面。这些机制在设计图上各自成立，但攻击者不会只挑选其中一层：恶意附件可以影响模型，模型可以请求一个权限过宽的 Tool，Tool Result 又可以污染 Memory 或远程 Agent Artifact，最后通过伪造运行事件或误导界面诱导人员批准。
...
> 阅读位置：完成 Agentic UI 四章后，安全主线依次经过威胁建模、Prompt Injection、最小权限与[纵深防御](/masterpiece-static-docs/08-安全与治理/04-纵深防御与人类控制.md)，再进入本章的系统性攻击链验证。AG-UI 与 A2UI 在这里都属于核心攻击面；Multi-Agent / A2A 的设计风险必须理解，但只有生产候选确实启用协作分支时，才执行对应远程链路的发布演练。
...
| AG-UI Adapter / A2UI / Native UI    | Provider Event、Canonical Event、生成文案、Surface、Action Context、URL | 投影运行状态、影响人员理解和授权            | Public State、Reducer、Trusted Preview、Approval、Action Gateway、Navigation |
...
### 6.5 Agentic UI：人类也处在攻击链中

UX 红队不是检查界面是否美观，而是验证人员能否看清当前事实、责任和外部效果：

- 伪造、重放或乱序 AG-UI Event，试图提前制造 `RUN_FINISHED`、覆盖终态或隐藏 `UNKNOWN`；
- 把 Provider Event 直接映射成业务成功，绕过 Canonical RunEvent 的提交与领域校验；
- 把 Client Shared State、DOM 或本地 Reducer 状态误当作服务端授权与业务事实；
...
验收的权威来源是 Application Server 的 Public State、Canonical Event Store、Approval Service、Action Gateway 和 Mock Ledger，不是 Provider Event、AG-UI / A2UI 本地状态或 DOM 中某个按钮的状态。
...
核心写路径必须运行 Tool、MCP、Memory、Agentic UI 与重复副作用演练，即路径 A、B、C、E、F。路径 D 只在生产候选启用 Multi-Agent / A2A 时加入；不采用远程协作不会阻塞核心成品，但必须保留其威胁模型与采用门槛。
...
flowchart LR
    ...
    RT -.->|"生产候选启用 Multi-Agent 时"| A2A["Fake Read-only Review Agent"]
    APP --> AGUI["Test AG-UI Adapter"]
    AGUI --> UI["Test Native / AG-UI Client"]
    AGUI --> A2UI["Test A2UI Renderer"]
    A2UI -->|"Action"| APP
...
#### 路径 E：伪造 AG-UI Event / A2UI Action → Human Trust Exploitation

- 入口：伪造、重放或乱序的 AG-UI Event，或者一条 Schema 可解析但试图声明 `approve_refund` Action 的 A2UI Surface；
- 目标：提前制造成功终态，或让客服误以为低风险补充信息已经完成退款审批；
- 期望防线：Provider → Canonical Adapter、只投影已提交 Event 的 AG-UI Adapter、Sequence / Dedup / Terminal Guard、Catalog / Action Allowlist、Action Gateway 与原生受信 Approval；
- 验收：Native Client 与 AG-UI Client 重放同一 Canonical Fixture 后状态一致；恶意 Surface 被拒绝或安全降级，Approval Record 不存在，Mock Ledger 无变化。
...
| Attack Surface Inventory | 核心 Tool、MCP、Memory、AG-UI Adapter、A2UI、Native UX、Identity 与 Telemetry 都有输入、权限、Sink 和 Enforcement Point；生产候选启用 Multi-Agent / A2A 时追加远程协作条目 |
...
- [A2UI v0.9.1 — Current Production Specification](https://a2ui.org/specification/v0.9.1-a2ui/)
- [A2UI — Renderer Development Guide](https://a2ui.org/guides/renderer-development/)
- [AgentDojo: A Dynamic Environment to Evaluate Prompt Injection Attacks and Defenses for LLM Agents](https://arxiv.org/abs/2406.13352)

> 资料与链接核验日期：2026-07-19。OWASP Agentic Top 10 和 MITRE ATLAS 属于会持续演进的威胁分类；MCP、AG-UI、A2UI 的规范与产品接口也可能更新。真实系统演练前必须重新核对当前规范、组织政策与书面授权。
