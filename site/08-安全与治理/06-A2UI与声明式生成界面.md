...
Resolution Desk 只把 A2UI 用于低风险的补充信息和证据选择，例如让客服确认退款原因、选择需要核对的政策片段。退款 Preview、Approval 与提交始终由固定的原生可信 UI 和专用服务端 Command 承担；任何 A2UI Surface 都不能创建、批准或执行退款。

> 版本核验日期：2026-07-15。本章以 A2UI `v0.9.1` **Current Production** 为实现基线；`v1.0` 仍为 **Candidate**。同时，官方仓库 README 仍将整体生态标记为 Early Stage Public Preview，因此“Current Production”只表示当前协议基线，不代表实现和 Renderer 已经没有 Breaking Change 风险。v1.0 增加的 `actionResponse`、`callFunction` / `functionResponse` 等双向 RPC 能力不属于本章的 v0.9.1 Contract。升级前必须重新核对规范状态、迁移指南和 Renderer 兼容性。
...
### 6.5 Action 必须重新 Authorization，高风险操作必须离开 A2UI
...
Resolution Desk 已有固定原生界面展示 Proposal、Approval、执行状态与恢复操作。系统只缺少一种可选能力：针对不同工单动态组合低风险澄清问题和证据列表。
...
4. 用 JSONL 读取器逐条解析 Envelope，在服务端 Adapter 与客户端 Renderer 两侧执行 Schema Validation；
5. 实现 `Map<surfaceId, Surface>` Reducer，支持四类 Envelope、唯一 `root`、前向引用、删除和资源上限；
6. 将 Catalog 映射到本地 React Component，禁止动态组件导入、Raw HTML 和 Props 全量透传；
...
完成标准包括：所有错误都能归因到 Transport、Envelope、Catalog、Graph、Content、Action 或 Domain 中的明确一层；任何失败都不会退回执行模型生成的 JSX/HTML，任何 A2UI Payload 都不能创建 Approval 或退款 Command。
...
协议只定义可交换的 UI 数据，不替代 Schema 与图验证、Catalog Allowlist、数据最小化、URL 与富文本策略、服务端 Authorization、a11y 或 Transport 恢复机制。在 Resolution Desk 中，它只负责低风险澄清与证据收集；退款 Preview 和 Approval 始终留在原生可信 UI。下一章进入 [Agent 安全评测与 Red Team](/masterpiece-static-docs/08-安全与治理/07-Agent安全评测与Red-Team.md)，把 Tool、MCP、Memory、Multi-Agent 与 UX 连接成可复现的完整攻击链。
