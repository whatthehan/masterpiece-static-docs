...
前端工程师通常熟悉这样的授权流程：浏览器带着用户 Session 调用 API，服务端根据用户与资源关系决定是否允许操作。Agent 应用多了一层委派：用户提出目标，Runtime 代表用户规划，Executor 再以某种凭证调用订单、邮件或支付服务。
...
1. 合法用户要求 Agent 查询另一租户的订单；
2. 低权限 Parent Agent 把请求委派给高权限 Worker；
3. CRM Reader 的结果被传给任意 Webhook；
4. 用户删除偏好后，旧内容仍可能从 Cache 或 Vector Index 命中。

验收要求是资源服务拒绝越权、跨域数据流被策略或环境阻断、删除状态可追踪，并且 Audit 能还原原始 actor、委派链、策略决策和最终结果。
