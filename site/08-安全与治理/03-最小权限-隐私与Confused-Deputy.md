...
- **Least Privilege**：只授予当前 Actor、Resource 和 Action 所需的权限，并限制 Audience、Scope 与期限。
- **Least Disclosure**：只把当前步骤必需的数据交给模型、工具和第三方服务。

三者分别缩小自主决策范围、可执行权限和数据暴露范围。只做最小权限，仍可能把整份客户档案送进 Context；只做最小披露，也无法阻止 Agent 重复执行有副作用的动作。
...
两次调用分别通过权限检查，并不代表组合后的数据流合法。若 Runtime 把客户档案发送到攻击者控制的 Webhook，问题不在单个工具的 Role，而在于数据来源、目的地和 Purpose 并不匹配。
...
跨 Agent 委派同样如此。Parent Agent 应向 Child Agent 传递缩减后的 Capability 和原始 Actor，而不是让 Child 继承 Parent 的全部权限。“来自内部”不等于“可以信任”。
...
把每次 Tool Call 约束为受限委派：可信 Session 注入 Actor 与 Tenant，Runtime 只传递当前任务所需的 Scope，Executor 获取绑定特定 Audience 的短期凭证，订单与支付服务再根据当前资源版本重新授权。随后覆盖四条攻击路径：
...
资源服务必须拒绝跨租户访问和扩大后的 Scope，数据流策略或执行环境必须阻断任意外发，删除 Canary 不能再被服务。Audit 只保存最小引用，但应足以还原 Actor、Delegate、Resource、Purpose、Policy Decision 和最终结果。测试通过前，常规业务 Run 不得调用 `commit_refund`。
