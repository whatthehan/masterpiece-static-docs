...
进入本模块时，Resolution Desk 已经能读取客服工单、查询订单、检索退款政策，并生成带证据的不可变退款提案。`commit_refund` 仍被写操作开关锁住：在开放这项能力之前，系统必须先证明不可信内容、错误模型输出和单层控制失效都不能直接变成退款。
...
## 实践：为 Resolution Desk 建立写操作准入边界
...
4. 将“无授权不读数据、无原生审批不提交、同一 Intent 不重复退款、密钥不进 Context、未知效果不盲目重试”等约束落实为 enforcement point；
5. 保持 `commit_refund` 写操作开关关闭，直到后续章节的 Injection、委派、纵深防御和可信 UX 验收全部通过。
