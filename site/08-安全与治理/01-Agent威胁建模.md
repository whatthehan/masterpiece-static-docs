...
进入本部分时，Resolution Desk 的三层主线已经形成第一条完整纵向切片：LLM 产生候选，Agent Runtime 查询订单、检索退款政策并生成带证据的不可变退款提案，Agentic UI 则通过公共状态、AG-UI 事件和受控 A2UI Surface 向用户展示过程并接收操作。`commit_refund` 尚未向常规业务 Run 开放：在允许调用 Mock Executor 之前，系统必须先证明不可信内容、错误模型输出、伪造界面输入和单层控制失效都不会直接造成退款。
...
```mermaid
flowchart LR
    U["用户或攻击者"] --> UI["Native UI / A2UI Renderer"]
    UI <--> A["Application Server<br/>AG-UI Adapter · Action Gateway"]
    A --> R["Agent Runtime"]
    R <--> M["Model Provider"]
...
5. 前述 Agentic UI Contract，以及本部分的 Injection、委派、纵深防御和 Red Team 门禁全部通过前，常规业务 Run 不得调用 `commit_refund`。
...
[上一部分：Agent Skills、动态工具发现与 MCP 扩展](/masterpiece-static-docs/07-工具-协议与行动控制/06-Agent-Skills与MCP扩展.md) · [下一章：Prompt Injection 与不可信内容](/masterpiece-static-docs/08-安全与治理/02-Prompt-Injection与不可信内容.md)
