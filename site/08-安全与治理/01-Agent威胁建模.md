...
设想一个售后 Agent：它能读取用户提交的附件，检索退款政策，查询订单，并在获得批准后发起退款。单看每个能力都很常见；风险来自这些能力被串在了一起——附件中的恶意内容可以影响模型，模型可以选择高权限工具，工具又能改变真实订单。
...
| 污染后续任务          | 恶意附件被摘要后自动写入 Memory             | Memory 写入门禁、来源与 TTL；删除和隔离机制              |

这里的关键是“必要控制点”必须位于确定性系统中。Prompt 可以降低模型犯错的概率，却不能承担授权、金额上限或网络出口这类硬约束。

## 4. 把安全目标写成不变量

安全不变量（security invariant）是任何输入、模型输出或故障情况下都必须成立的约束。每条不变量应同时写出 enforcement point 和验证方式。
...
为正在构建的 Agent 应用提交以下内容：

1. 一张包含模型、知识、工具、身份、外部系统和遥测的数据流图；
2. 至少十项资产及其敏感级别；
3. 五条具体滥用故事，覆盖越权、注入、重复副作用、资源耗尽和数据保留；
4. 六条安全不变量，每条写明 enforcement point、失败状态和自动化测试；
5. 使用 OWASP 清单进行一次遗漏检查，并说明不适用项的理由。

如果某条约束只能写成“要求模型不要这样做”，说明系统还没有为它建立安全边界。

## 本章小结

Agent 威胁建模关注的是一条完整效果链：不可信内容如何进入 Context，模型如何提出动作，权限如何被委派，Executor 如何接触真实系统，结果又如何进入状态与日志。完成这张图后，下一章将沿最常见的入口继续分析 [Prompt Injection 与不可信内容](/masterpiece-static-docs/08-安全与治理/02-Prompt-Injection与不可信内容.md)。
