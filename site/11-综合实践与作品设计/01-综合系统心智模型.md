...
flowchart TD
    ...
    APP["Agent Application Server<br/>Thread · Run · Item · State Machine"]
    CTX["Context / Knowledge<br/>Source · ACL · Freshness · Memory"]
    MODEL["Model Provider"]
    REMOTE["Remote Agent Systems<br/>AgentCard · Task · Artifact"]
    ...
    UX <--> |"AG-UI events / controls<br/>A2UI payload"| APP
    APP <--> CTX
    APP <--> |"versioned request / candidate items"| MODEL
    APP <--> |"A2A delegation / task updates"| REMOTE
    APP --> POLICY
...
| Application Server   | Thread/Run/Item、状态机、预算、取消、恢复         | 决定资源服务的最终授权          |
...
15. 修复版本经过 Offline Eval、Shadow、Canary 和 Rollback 演练后才扩大流量。
...
8. 终态不能继续产生新业务动作；未知效果只能核对、按原 Intent 幂等收敛或转人工。
...
1. **任务与评测基线**：明确输入、Outcome、禁止行为、简单 Baseline 和案例集；
...
5. **受控行动**：身份、授权、Approval、幂等、Receipt 与 Sandbox；
6. **持久与生产**：Checkpoint、Reconciliation、SLO、Canary、Rollback 与真实 UX；
7. **场景专项**：只有在 Baseline 证明收益后，再加入 Multi-Agent、Computer Use 等复杂能力。
...
在不查看本章的情况下重画系统图，并选择一个带写操作的真实任务，逐步说明候选如何成为 Command、Timeout 后为何不能直接 Retry、Worker 如何恢复、UI 如何展示未知效果，以及新版本如何安全进入流量。
