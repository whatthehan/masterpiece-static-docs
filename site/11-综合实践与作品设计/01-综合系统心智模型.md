...
完成前面的章节后，Agent 应用可以沿三条知识主线重新看成一套熟悉的软件系统：LLM 负责处理开放语义，Agent Runtime 负责组织 Context、状态、工具与预算，Agentic UI 负责让人类观察、控制并恢复任务；Eval、安全、可靠性与 Operations 则持续验证这三层能否进入生产。
...
8. **场景专项**：理解 Multi-Agent 与 A2A 的状态边界；只有在 Baseline 证明收益后，才把协作、Skills、Dynamic Discovery 或 Computer Use 加入生产路径。
