...
只读售后分析 Skill 固定 Publisher、Version 与 Digest，按 progressive disclosure 加载；Dynamic Tool Discovery 先按 actor、tenant 和 risk 过滤，再展开少量 Query Tool Schema。Skill 指令和发现结果都不能把 `commit_refund` 加入动态能力面。
...
[返回全书首页](/masterpiece-static-docs/index.md) · [查看八周构建路径](/masterpiece-static-docs/11-综合实践与作品设计/04-八周学习与实践路径.md) · [查看综合能力自测](/masterpiece-static-docs/11-综合实践与作品设计/02-综合能力自测.md)
