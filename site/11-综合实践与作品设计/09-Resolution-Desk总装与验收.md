...
前面的章节分别建立了 LLM 与 Model Adapter、Agent Loop、Context Builder、Knowledge、Tool Contract、Authorization、AG-UI、A2UI、Agent UX、Recovery 和 Eval。本章不再增加新的核心概念，而是把 LLM 底层、Agent 系统与 Agentic UI 三层装配成一条可以运行、失败、恢复并接受复查的产品纵切面；安全、可靠性与 Eval 证据贯穿整条链路。
...
## 10. 先验证核心拓扑与 UI 契约，再验收进阶能力

Production Topology Test 与 Agentic UI Contract Test 是核心总装门禁；框架与 Multi-Agent 测试按采用状态执行：
...
3. **Framework Ejection Test（采用候选框架时）**：手写 Runtime、AI SDK Adapter 与实际候选 Adapter 使用同一 `AgentRuntimePort`、Domain、Canonical Event、Dataset 和对应 durability tier 的故障矩阵。替换 Adapter 不修改 Policy、UI reducer 与权威 Outcome。
4. **Multi-Agent Admission Test（评估或启用协作扩展时）**：只对证据量大、来源独立的 Case 启动 Policy Evidence Worker 与 Case Evidence Worker。Child 只返回不可变 Artifact，Coordinator 保留唯一 ownership；与固定 Workflow、单 Agent 和 best-of-N 做等预算比较，没有净收益时删除。
...
前九条是核心成品门禁；A2A 风险复核只在生产候选采用远程协作时进入发布门禁。Skills、Dynamic Tool Discovery、MCP Apps/Tasks 与其他场景扩展也只有在采用时进入对应门禁。
