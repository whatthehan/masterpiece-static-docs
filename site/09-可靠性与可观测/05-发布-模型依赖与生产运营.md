...
Resolution Desk 已具备从退款请求、证据检索、AG-UI 状态同步、A2UI 澄清、原生 Approval、Command、未知效果核对到跨 Worker 恢复的完整路径，并能用 Trace、SLO 和成本证明当前行为。最后一步，是让新的 Model、Prompt、Context Builder、Tool Schema、Policy、Public Event、UI Catalog、Retrieval Index 和 Runtime 接受同一套发布准入与渐进放量策略。只回滚容器镜像，未必能恢复上一版本的行为。
...
type BehaviorBundle = {
  ...
  publicEventSchemaVersion: string;
  agUiContractVersion: string;
  a2uiCatalogVersion: string;
...
`BehaviorBundle` 描述线上行为依赖，每个 Run 在创建时记录一份，长期 Run 还要记录后续显式迁移。`knowledgeSnapshotVersion` 固定可用知识集合，`retrievalIndexVersion` 与 `retrievalSchemaVersion` 固定检索派生物和字段语义；只有索引能够从同一 Snapshot 确定性重建时，回滚才可以省略实体快照。Skill、Hook、MCP Server 配置等扩展由 `extensionSetVersion` 统一引用。Public Event、AG-UI Contract 与 A2UI Catalog 分别版本化，避免后端升级后旧客户端误解终态、控件或 Action。

`EvaluationManifest` 描述比较所用的 Dataset、Grader 与环境 Fixture。一次发布评测同时记录 Behavior Bundle 和 Evaluation Manifest，避免把“系统行为版本”与“如何测量该行为”混在同一个类型里。Feature Flag、Prompt、知识快照和模型路由都是发布内容，不能以“只是修改配置”为由绕过 Eval、审批和回滚。

### 候选生成与生产发布必须隔离

Behavior Bundle 可以由工程师、Coding Agent 或 Prompt Optimizer 提出，但来源不会改变发布门禁。运行中的 Agent 只能记录 Observation、Feedback 和 Change Proposal，不能热修改当前 Run 使用的 Prompt、Context Builder、Tool、Policy、Grader 或 Bundle。任何自动生成的改动都必须形成引用 `baseReleaseId`、Finding、Diff、Change Surface 和风险等级的 Candidate Behavior Bundle。

Optimizer 只访问经过脱敏且获准使用的 Trace、Development 与 Regression Dataset，并在隔离环境写入候选 Artifact；它不能读取私有 Holdout、修改 Grader 和安全门禁，也不能持有生产写权限或发布凭证。Tool Capability、Authorization 与 Policy 变更还需要相应责任人审查，不能由平均分自动决定。Signal 如何形成 Finding、DSPy/GEPA 等 Optimizer 如何进入实验，以及候选怎样接受独立评测，见[受控改进：从生产证据到候选 Behavior Bundle](/masterpiece-static-docs/09-可靠性与可观测/07-受控改进-从生产证据到候选Behavior-Bundle.md)。
...
模型自报“这个任务很难”只能作为候选信号，不能决定访问权限或高风险执行路径。可以先用规则覆盖不可协商的约束，再由轻量分类器（Classifier）在获准候选中选择。路由结果记录如下：
...
Router 本身需要独立 Eval。在同一 Dataset 上比较固定强模型、固定低成本模型和路由策略，并报告 Protected Slice 的成功率、误路由率、升级率、P95/P99、单位成功任务成本与 Provider 集中度。只看平均成本，会掩盖高风险 Slice 被错误下放的问题；只看总体成功率，也会掩盖 Router 把困难任务全部转人工的情况。
...
运行版本化 Dataset、Contract Test、故障用例和安全用例。总体分数不能掩盖关键 Slice 的退化。

### Shadow

复用经过脱敏且获准使用的生产输入，比较新旧版本的候选 Outcome、Trajectory、Latency 和 Cost。写工具必须替换为无副作用的 Executor；“执行后再删除”不属于 Shadow。
...
Open 时停止创建相关的新调用，Half-open 时只允许有限的探针请求。降级必须保留任务语义：研究任务可以返回有来源的 Partial Result；高风险写动作不能静默切换到未经评测的模型；效果未知的 Command 仍通过独立 Reconciliation 通道核对。
...
将 Model Route、Prompt、Context Builder、Knowledge Snapshot、Retrieval Index、Toolset、Policy、Workflow 与 Runtime 固定为一个 Behavior Bundle。针对同一候选版本，实现 Offline Eval、无副作用 Shadow、受限 Canary、Ramp、Rollback、Circuit Breaker、Kill Switch，以及已验证的 Fallback 或明确的非 Fallback 路径。

再为一个低风险只读 Slice 建立 Model Cascade，并与固定强模型和固定低成本模型比较。随后注入误路由、Provider 限流、Tool 延迟、ACK 丢失、用户 Cancel 和 Rollback，并将 Drain 的行为不变量写入发布策略。Worker 终止与迁移演练留待下一章的拓扑实践。
...
Agent 发布的对象是一套版本化行为系统。Offline Eval、Shadow、Canary、Ramp、Rollback、Circuit Breaker 和 Kill Switch 共同约束版本更替：新版本只接收受控流量，旧 Run 仍固定原 Behavior Bundle，效果未知的 Command 仍要通过对账获得明确结论。下一章将把这些发布语义放入可扩缩、可迁移的 [生产拓扑](/masterpiece-static-docs/09-可靠性与可观测/06-生产拓扑-部署与迁移验收.md)，检查 Application Server、Worker、Queue、Store 与 Tool Gateway 如何共同承载真实运行生命周期。
