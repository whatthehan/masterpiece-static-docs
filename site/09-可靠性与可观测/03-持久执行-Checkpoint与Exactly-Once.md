...
持久执行保存的是控制事实、版本和所有权，而不是进程连续性。Checkpoint 使 Run 可恢复，Event History 使控制逻辑可 Replay，幂等与权威查询让重复执行的 Activity 收敛到同一权威结果，Fencing/CAS 则防止双写。下一章将用 [Trace、SLO 与成本](/masterpiece-static-docs/09-可靠性与可观测/04-Trace-SLO与成本.md)观察这套系统是否真的可靠。
