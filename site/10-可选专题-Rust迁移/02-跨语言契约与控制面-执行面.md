# 02 · 跨语言契约：连接 Node.js 控制面与 Rust 执行面
...
因此，跨语言迁移的核心交付物不是 Rust 代码，而是一份双方都能执行、测试和演进的协议契约。若 Resolution Desk 没有测量出需要迁移的本地瓶颈，本章只作为边界设计的深入阅读。
...
可以从 JSON Schema 或 Protobuf 生成 TS/Rust 类型，但生成器只能保证结构。金额单位、时间边界、Unicode 处理、权限来源和失败语义仍需写入规范，并用 Fixture 验证。
...
金额、ID、Deadline、Missing/Null 与 Canonical Hash 最容易出现“类型检查通过，业务语义却发生漂移”的问题，必须进入 Golden Vector。
...
生成大量输入，验证 Round-trip、无溢出、大小上限、Canonicalization、去重和幂等不变量。Property Test 不能替代业务 Golden Case，两者覆盖的风险不同。
...
比较对象不能只有一段相同的 JSON，还应包括真实业务状态、错误分类、Span 关系、Audit 字段、Latency 分布、资源消耗和副作用次数。
...
跨语言迁移是一项协议与证据工程。稳定 Wire Semantics、可验证身份、共享错误状态机和多层测试，才能证明 Node 控制面与 Rust 执行面仍属于同一个系统。完成实验后，使用 [Rust 迁移验收清单](/masterpiece-static-docs/11-综合实践与作品设计/07-Rust迁移验收清单.md) 检查采用条件；没有收益时保留 TypeScript + Node 是正常且正确的工程结论。
...
[下一部分：综合系统心智模型](/masterpiece-static-docs/11-综合实践与作品设计/01-综合系统心智模型.md)
