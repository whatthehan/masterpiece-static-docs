...
分别将正确证据放在 Context 开头、中间和末尾，再逐步增加干扰。每种条件运行多个 Trial，记录：
...
随后对历史做一次 Compaction，重放同一 Task，检查精确金额、Approval 状态、未完成事项和引用关系是否仍然保留。
...
[下一章：Grader、Trial 与统计](/masterpiece-static-docs/04-评测与实验科学/01-Grader-Trial与统计.md) · [查看完整转型路线](/masterpiece-static-docs/01-导读/05-从学习到转型的完整路线.md)
