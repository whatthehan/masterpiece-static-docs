...
上下文工程（Context Engineering）的目标不是塞满窗口，而是为每次推理构造最小、充分且可追溯的 Context。
...
具体计数、截断和推理 Token 的语义由目标模型与 API 决定，实施时应以官方文档和实际 `usage` 为准。
...
在较长的 Context 中，相关信息可能被无关内容包围，也可能与旧版本产生冲突。研究和实践均表明，模型利用不同位置的信息时并不总是同样可靠。
...
- **贪心解码（Greedy / Argmax）**：每一步都选择概率最高的候选。
- **温度（Temperature）**：调整概率分布的集中程度。
...
降低 Temperature 可以减少某些输出变化，但不能修正错误的 Context，也不能替代多 Trial Eval。
...
最终保留 Context Pack、排列条件、Recorded Output、评分和 Compaction 对照表。[Context Engineering](/masterpiece-static-docs/06-上下文-知识与记忆/01-Context-Engineering.md)实现 Context Builder 时将重放这些 Fixture，用来验证选择策略，而不是重新设计一批测试案例。

## 常见误区

- 标称一百万 Token 就表示任意位置都能可靠回忆。
- 把全部资料放入 Prompt 总比 Retrieval 更可靠。
- 更低的 Temperature 必然提高事实正确率。
