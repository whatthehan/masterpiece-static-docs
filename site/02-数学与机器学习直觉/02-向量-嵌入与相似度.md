...
Resolution Desk 需要从售后政策、订单事实和历史工单中找到当前客服有权使用的证据。本章只建立 Knowledge Fixture 与检索基线，不把检索接入 Agent：政策文档保留版本、生效时间、租户和来源，查询结果保存为可重放的 Retrieval Fixture。第 06 模块再将同一批材料接入正式 Knowledge Plane。
...
围绕 Resolution Desk 的“物流延误退款”准备 24 份短文档，包含以下变体：
...
为 8 个售后查询标注相关文档，分别运行 keyword、dense 和 hybrid retrieval。可以使用独立脚本、Notebook 或托管检索控制台；本实验不需要 Agent Runtime，也不执行退款。记录：
...
实验重点不是找一个最高 Cosine Similarity，而是确认：必要证据是否召回、无权内容是否在检索前被排除、过期版本是否可识别、恶意文本是否始终作为不可信数据处理。保留文档、Query、相关性标注和结果列表；它们是 Resolution Desk 后续 RAG、ACL 与 Freshness 测试的共同输入，而不是一次性 Demo。
