...
本章把前两章的 Trial Fixture 与 Knowledge Fixture 组织成四个数据切片（Dataset Slice）：标准输入、表达变化、信息缺失或冲突、安全与权限边界。此时只定义数据分布、标签和版本，不要求系统已经能够检索、调用 Tool 或提交退款；这些 Fixture 将成为后续每次能力增量都必须回放的稳定基线。

## 1. 模型优化的是代理目标

训练过程在有限数据上优化损失函数（Loss Function）。Loss 降低，说明模型对训练数据和目标函数的拟合有所改善；产品真正关心的却是另一组结果：事实是否正确、任务是否完成、权限是否得到遵守、是否产生重复副作用。
...
训练目标只是产品目标的代理指标（Proxy Metric），两者之间隔着数据、运行环境、工具和业务策略等多个层次。即使模型很擅长生成符合人类偏好的回答，也不代表它能够访问当前订单状态，更不代表它有权提交退款。代理指标改善时，必须用真实业务结果验证目标是否同步改善。
...
这张表是排查问题的基础。政策未进入 Context，不应试图通过提高 Temperature 解决；订单状态已经过期，不应通过微调模型解决；支付 Tool 越权，也不应依赖“更安全的 Prompt”补救。
...
### 交互偏移与非平稳性（Interaction Shift / Non-stationarity）
...
Agent 还可能利用评分器的代理指标漏洞，这种现象称为评分器投机（Grader Gaming）。例如，Grader 只检查最终文本是否包含“已核对支付状态”，系统便可能直接生成这句话，而没有真正查询支付服务。可靠的 Grader 应尽量检查权威环境状态和结构化 Trace，使通过评测的最短路径与真正完成任务的路径一致。
...
- 输入长度、语言、渠道和任务类别变化。
- Retrieval 的无结果率、召回率替代指标（Recall Proxy）和过期来源比例。
