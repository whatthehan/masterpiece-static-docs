...
代码评审 Agent 已经能够读取 Pull Request、运行测试并生成 Review；安全审计 Agent 则由另一个团队维护，部署在独立网络中，拥有自己的模型、策略与工具。前者需要把一项安全复核工作委派给后者，但不应该知道对方的 Prompt、Memory、Tool 实现或内部执行步骤。
...
> 版本核验日期：2026-07-15。本章以 Linux Foundation A2A 项目的 `v1.0.0` 为基线，该版本于 2026-03-12 发布，官方定义为首个 stable、production-ready 版本。A2A JavaScript SDK 的稳定通道此时仍实现 v0.3，v1 支持位于 `@a2a-js/sdk@next` 预发布通道，因此本章使用协议级 TypeScript 与 `fetch` 伪代码，不引入预发布依赖。
...
- 识别开放协议生态的版本漂移，并用 Adapter 与 Contract Fixture 隔离差异。

## 1. 从代码评审委派开始

设想一个熟悉的工程工作流：

1. 用户要求代码评审 Orchestrator 检查一个 Pull Request。
2. Orchestrator 完成常规 Review，发现认证中间件有高风险改动。
3. 它发现一个独立 Security Audit Agent，并确认对方支持代码安全审计。
4. Orchestrator 只发送经过最小化处理的 Diff、依赖信息和审计目标。
5. Security Agent 创建 Task，流式返回进度与审计报告 Artifact。
6. 若缺少 Threat Model，它进入 `INPUT_REQUIRED`，等待补充信息。
7. Orchestrator 将结果呈现给用户，并把最终报告关联回原始 Review。

```mermaid
flowchart LR
    U["工程师 / Review UI"]
    O["Code Review Orchestrator"]
    S["Security Audit Agent"]
    T["Repository / Scanner / CVE Data"]
...
这张图中有四条不同的协议边界。A2A 只负责 Orchestrator 与独立 Agent 的协作，不负责 Security Agent 如何调用扫描器，也不规定 Review UI 如何渲染结果。
...
一个系统可以同时使用四者：Security Agent 内部通过 MCP 访问扫描工具，外部通过 A2A 接受委派；Orchestrator 通过 AG-UI 与前端通信，并把 Security Agent 返回的 A2UI 数据交给受信 Renderer。

不能因为它们都使用 JSON 或 Streaming 就把它们当成竞争协议。判断边界的关键问题是：当前对端是一个 Tool、一个自主 Agent，还是一个面向用户的 UI Runtime。
...
- `tenant`：可选的服务端路由值，存在时由 Binding Adapter 作为 Path Parameter 安全编码并带入请求。

客户端应选择自己支持的第一个 Interface，而不是看到 URL 就猜测协议。`protocolVersion` 与 Agent 自身的 `version` 也不是同一概念：前者决定 Wire Contract，后者描述服务实现版本。

### AgentSkill

AgentSkill 描述 Agent 能完成的一类任务，通常包含稳定 ID、名称、描述、Tags、示例，以及可覆盖全局默认值的 `inputModes`、`outputModes` 和安全要求。

Skill 是路由与能力匹配的依据，不是授权证明。Security Agent 声明 `secure-code-review`，只说明它声称理解这类工作；Orchestrator 仍需验证 Agent 来源、允许的代码仓库、数据分级和委派范围。
...
Security Agent 若只回答“该语言不在支持范围内”，可以返回 Message。若要排队、运行扫描器、请求补充信息并生成报告，则应创建 Task。
...
Artifact 由 `artifactId`、可选名称与描述、一个或多个 Parts 组成。扫描进度属于 Status Message；最终 SARIF、Markdown 报告或风险 JSON 才适合作为 Artifact。
...
```text
context: PR #842 security review
  task A: initial audit                 → COMPLETED
  task B: re-check after remediation   → COMPLETED
  task C: dependency license review    → WORKING
```

终态 Task 不可复活。用户要求“基于刚才的报告检查修复结果”时，应在同一 `contextId` 下创建新 Task，并通过 `referenceTaskIds` 引用旧 Task。这样每份 Artifact 都能追溯到确切输入和执行单元。

不要把 `contextId` 当成授权边界或数据库租户字段。它是由服务端解释的 opaque identifier，资源访问仍需基于已认证 Actor 和服务端授权模型。

## 6. Task 状态机

下图是本章安全审计场景的典型主路径，用于突出 Interrupted 与 Terminal State 的差别，不穷举服务端可能实现的所有迁移。
...
sequenceDiagram
    ...
    O->>S: SendStreamingMessage(minimized diff)
    S-->>O: Task(SUBMITTED)
    S-->>O: TaskStatusUpdate(WORKING)
    S-->>O: TaskArtifactUpdate(partial findings)
    S-->>O: TaskStatusUpdate(INPUT_REQUIRED)
    Note over O,S: peer / binding 可能在 Interrupted State 关闭当前流
    O->>S: SendStreamingMessage(same taskId, threat model)
    S-->>O: Task(current state as first event)
    S-->>O: TaskStatusUpdate(WORKING)
    S-->>O: TaskArtifactUpdate(final report)
...
async function streamSecurityReview(
  endpoint: AgentInterface,
...
A2A v1 核心事件也没有提供跨 Binding 通用的单调 Sequence 或 Event ID。若 Peer 没有通过 Extension Metadata 提供稳定游标，Client 只能使用已知字段和内容摘要做尽力去重，并最终以 GetTask 返回的完整 Task 与 Artifact 收敛状态，不能声称实现了无损重放。
...
如果 Security Agent 已创建外部工单或写入审计系统，仍需上一章的 Idempotency、Receipt 与 Compensation 机制。A2A 统一交互语义，不提供跨 Agent Exactly-once Side Effect。
...
AgentCard 可以使用 JSON Web Signature（JWS）证明来源与完整性。Client 应在存在签名时验证可信 Key、过期与撤销状态，但签名只说明“这张 Card 来自哪个 Provider 且未被篡改”，不说明当前用户可以把仓库代码交给它。
...
每一跳都应限制 Audience、Scope、Tenant、Repository、有效期和可执行动作。不要把用户的长期 Token 原样透传给 Remote Agent。
...
跨组织安全审计不应默认发送整个 Repository。优先发送：

- 相关 Diff 与必要上下文；
- 已脱敏的配置结构；
- Dependency Manifest，而非本地 Credential；
- 明确的数据使用目的、保留期限和输出模式。
...
开放协议的规范、Extension 和 SDK 不会总在同一天完成迁移。2026-07-15 核验时，A2UI 的 A2A Extension 页面仍有若干旧式示例，与 A2A v1.0.0 的权威定义不一致：
...
对 A2UI over A2A，领域层只保留 A2UI Message；Wire 差异由 Adapter 处理：
...
兼容旧 Peer 时，不要在业务代码中散落条件分支，也不要默认同时发送新旧 Header。应为已知 Peer 配置明确 Adapter，并维护 Contract Fixtures：
...
Contract Test 至少验证：Header 名称、协议版本、Part One-of、`mediaType` 位置、枚举值、Task 首事件、Artifact 合并和未知字段处理。这样 Extension 更新时，只需修改 Adapter 与 Fixture，不会污染 Task 领域模型。
...
| 首个 Stream Event 不是 Task       | 协议错误，关闭 Stream                                       | 猜测 Task ID 后继续                |
...
1. 创建本地 Mock Security Agent，提供 v1 AgentCard 与 `secure-code-review` Skill。
2. 实现 `POST /message:stream`，依次输出 Task、WORKING、Artifact Chunk 和 COMPLETED。
3. 实现 `GET /tasks/{id}` 与 `GET /tasks/{id}:subscribe`。
4. 在 Orchestrator 中验证 AgentCard、选择 HTTP+JSON v1 Interface，并发送最小化 Diff。
5. 模拟 `INPUT_REQUIRED`，用相同 `taskId` 与 `contextId` 补充 Threat Model。
...
- 未经测试的 Extension Shape 无法穿过 Adapter。
...
协议标准化不会消除分布式系统风险。生产实现仍需对 SSE 断线执行 GetTask Reconciliation，对可能重复或丢失的 Webhook 使用 Inbox、去重与 GetTask 对账，同时实施 SSRF 防护；每个 Operation 都需重新授权，Orchestrator 也必须限制委派循环、预算和数据披露。开放生态还会发生 SDK 与 Extension 的版本漂移，因此固定权威 Proto、隔离 Adapter 并维护 Contract Fixtures，是实现互操作性的必要部分。

下一部分进入 [Agent 威胁建模](/masterpiece-static-docs/08-安全与治理/01-Agent威胁建模.md)，将这些跨系统风险收敛为可维护的 Trust Boundary 与 Abuse Case。
