...
下一章进入 [Agent Skills、动态工具发现与 MCP 扩展](/masterpiece-static-docs/07-工具-协议与行动控制/06-Agent-Skills与MCP扩展.md)，辨别可发现、可加载、可调用与业务获准之间的边界。Agentic UI 第一阶段已在第 05/09–10 章建立；完成本部分后，第 08 部分将先补齐安全前置，再进入 Agent UX 与 A2UI。
