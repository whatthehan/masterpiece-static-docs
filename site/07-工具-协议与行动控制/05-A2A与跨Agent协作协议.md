...
Resolution Desk 可以独立处理大多数售后工单；当退款金额超过阈值，或者订单事实与政策证据冲突时，需要把风险复核委派给一个独立部署的 Risk Review Agent。Resolution Desk 不需要知道对方的 Prompt、Memory、Tool 实现或内部执行步骤，远端 Agent 也不应获得退款执行权限。
...
Contract Test 至少验证：Header 名称、协议版本、Part One-of、`mediaType` 位置、枚举值、Message-only 关闭规则、Task Lifecycle 首事件、Artifact 合并和未知字段处理。这样 Extension 更新时，只需修改 Adapter 与 Fixture，不会污染 Task 领域模型。
...
Resolution Desk 已能在本地读取证据、生成冻结 Proposal，并在隔离 Harness 中验证 Approval 与幂等退款语义；普通 Run 仍停在 `command_ready`，写操作开关保持关闭。普通工单不需要第二个 Agent，只有金额超过阈值或证据冲突时才进入只读风险复核分支。
...
下一章进入 [Agent Skills、动态工具发现与 MCP 扩展](/masterpiece-static-docs/07-工具-协议与行动控制/06-Agent-Skills与MCP扩展.md)，区分可加载方法、可发现能力、内嵌 View、长时协议 Task 与业务授权。
