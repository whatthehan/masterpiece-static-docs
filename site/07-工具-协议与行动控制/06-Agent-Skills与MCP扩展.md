...
Claude Code、Codex 等 Agentic Harness 可以按需加载一套工作方法，也可以从大量 Tool 中只取当前任务需要的几个定义；MCP Server 还可以返回交互界面，或者把长时间操作表示成可恢复查询的任务句柄。这些能力共同改善了 Agent 的可组合性，却分属不同边界。
...
`SKILL.md` 的 `name` 与 `description` 是必填字段；`license`、`compatibility` 和 `metadata` 是可选字段。`description` 不只是介绍文案，它承担触发检索的职责：过宽会误触发，过窄会漏掉应该使用 Skill 的任务。
...
这段示例中的 `metadata.version` 是维护方约定，不是规范要求的统一版本字段。Agent Skills 规范允许自定义字符串 metadata，但没有定义跨平台通用的发布版本和内容 Digest 语义。生产 Host 因此不能只读 `version: "1.2.0"` 就认为内容未被替换。
...
Host 启动时只需索引少量 metadata；匹配任务后再加载 `SKILL.md`；详细参考资料和脚本只在实际步骤需要时读取。它与把数十份手册一次性拼进 System Prompt 的区别，不只是节约 Token：
...
Progressive Disclosure 不是权限控制。未加载某个文件只表示当前 Context 中没有它，不表示进程无法读取它；真正的文件、网络和 Tool 权限仍由 Sandbox、Credential Scope 与 Policy Gate 控制。
...
评测集应包含 should-trigger、should-not-trigger、边界输入和对抗输入；同一 Case 分别运行 with-skill 与 baseline，比较任务通过率、Token、耗时和失败类型。安全相关断言不能只检查最终文字，还要检查 Trace 中是否读取了未授权 Reference、执行了脚本或尝试调用 Command。
...
Anthropic Tool Search 是一种 Provider 实现：延迟加载的 Tool 不进入初始 Context，搜索返回 `tool_reference` 后才展开完整定义。其他 Runtime 可以用 BM25、Embedding、规则索引或 MCP Catalog 实现同一模式。Provider API 不是应用架构本身，核心链路应保持可移植：

OpenAI Agents SDK 在 Responses 路径中提供了另一种具体实现：`toolSearchTool()` 负责检索，Function Tool 或 Hosted MCP Tool 可以标记 `deferLoading: true`，`toolNamespace()` 则把一组延迟函数作为命名空间按需展开。这条能力是 Responses 专用路径；官方文档明确说明 AI SDK Adapter 当前不支持 Deferred Responses Tool Loading。因而“Adapter 能调用 OpenAI 模型”不能推导出“Deferred Tool Loading 也能经由该 Adapter 工作”。采用前需要验证 API、Model 与 Adapter 的组合，并继续保留 Catalog 可见性、Authorization 和 Contract Fixture。
...
| Execute   | 结构校验后的参数、Deadline、Idempotency Key             | 跳过语义校验、Approval 或副作用 Gate     |
| Observe   | Typed Result、Receipt、Trace 与指标                | 把不可信 Tool Result 当成系统指令       |

某些 Provider 会把 Shortlist 与 Load 合并为一次服务端操作。此时应用必须在提交 Catalog 前保证其中每个延迟 Tool 都已通过可见性授权，并在真正执行前再次检查资源和 Intent；不能因为 API 自动展开定义而省略 Gate。
...
- canonical name 与稳定 namespace；
- 正向用途与明确的 non-goals；
- `query | draft | command` 分类；
- tenant、数据分级和所需 scope；
- Schema version 与 capabilities digest；
...
- 名称相近但业务语义不同的 Tool；
- 正确行为是回答“不需要调用 Tool”的 Case；
- 只能看到某一 tenant 能力的相同请求；
- 恶意 Description、重复 canonical name 和过大 Schema；
- Discovery 后 Tool 被撤销或版本发生变化；
- 搜索无结果、低置信度与候选冲突。

以 eager loading 作为 Baseline，固定模型、任务、总推理预算和 Tool 可用范围，再比较 Discovery 方案。否则 Token 下降但错误动作上升，仍然不是工程改进。
...
Resolution Desk 已通过 MCP 读取订单、物流和政策，并用本地 Tool Registry、Authorization、Approval 与 Executor 控制退款副作用；高金额或证据冲突 Case 还可以通过 A2A 请求只读风险复核。新增扩展不能改变退款 Command 仍归本地应用所有的事实。
...
- Tasks Server 不可用时，主流程回退到普通只读查询或转人工，不阻塞退款 Case 的基本处置。
...
| Auth Extension    | Service Principal 或企业身份可被 Trace 到明确 Issuer 与 Scope        | 正确 Token 仍无法绕过 tenant、resource、intent 与 Approval Gate |
...
Agent Skills 把工作方法做成可渐进加载的开放目录，但其内容和脚本必须像依赖包一样固定版本、Digest 与运行权限。Dynamic Tool Discovery 把 Tool Catalog 变成可检索能力面，真正安全的链路仍要在发现、加载和执行之间保留 Authorization。MCP Apps 为 Server 提供标准内嵌 View，Sandbox、CSP 和 Host Proxy 决定其安全上限；MCP Tasks 为长时协议操作提供 Handle，却不接管应用 Workflow。Authorization Extensions 解决 Token 获取方式，业务资源与副作用仍由应用授权。
...
[下一部分：Agent 威胁建模](/masterpiece-static-docs/08-安全与治理/01-Agent威胁建模.md)
