...
为每一步输出 Actor–Resource–Action Policy 结果和 Proposal State。只要 Hash、Actor、Tenant、Resource Version 或 Expiry 不匹配，隔离测试 Executor 就不能收到 Command。合法审批只能让原始订单、金额和业务 Intent 进入 `command_ready`；常规业务 Run 此时仍不能调用 Mock Executor，真正执行前还必须重新授权。
