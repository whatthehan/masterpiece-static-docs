...
依次比较 Sparse、Dense、Hybrid 和 Rerank，分别报告 Retrieval Recall、ACL Violation、Stale Evidence Rate、Citation Support 和最终任务成功率。复现一次“global top-k 后过滤”造成的 Recall Starvation；正确实现中，另一 Tenant 的高相似政策既不进入候选，也不会挤掉当前 Tenant 的有效证据。
