...
在 Claude Code 或 Codex 中，把代码检索、实现和复核交给不同的 Subagent，常常能够减少单个 Context 中的噪声，也能并行处理互不依赖的工作。产品中的多智能体（Multi-Agent）系统使用了相似思路，却多出一组不能依靠 Prompt 解决的问题：谁拥有任务、谁能使用哪些工具、子任务失败后如何恢复、多个结果冲突时由谁裁决。
...
- 划分 Planner、Worker、Strategist、Verifier 的能力与状态写入边界。
...
4. 汇合规则能否由代码、独立验证器（Verifier）或人工明确表达？
...
Generator 产生候选，Evaluator 按明确标准指出缺陷，再进入有限轮修订。Evaluator 必须与 Generator 的失败模式尽量独立，并设置最大轮数；否则容易出现 Grader Gaming 或无限往返。
...
三种机制都需要明确的协作边界契约（Coordination Contract），但只有 Handoff 会转移 Active Owner。通用信封可写成：
...
Delegation 中，Parent 仍是 Active Owner，子调用返回后继续原 Run。A2A Task 中，远端持有自己的 Task 生命周期，本地只通过协议句柄跟踪它，并在收到 Artifact 后独立验收。真正的 Handoff 还需要一份所有权转移契约（Ownership Transfer Contract）：
...
只有旧 Owner、当前 Epoch 和新 Owner 以原子方式完成持久化后，Handoff 才正式生效；旧 Owner 随后产生的写入必须因 Epoch 过期而被拒绝。`allowReturn`、升级目标和最大跳数应在移交前确定，不能由新 Agent 在自然语言对话中自行扩张。

无论采用哪种机制，接收者都不能把自然语言中的“可以处理退款”解释为执行权限。`allowedCapabilities` 只是本次协作的能力上限，实际权限仍由接收方根据 Workload Identity、Original Actor、Tenant、Purpose 和资源状态重新判定。
...
- **Parent Run**：持有用户目标、总预算、子任务目录、取消状态与最终结果所有权。
- **Child Run**：持有独立 Context、模型与工具版本、子预算、Attempt、Checkpoint 和终态。
...
### 6.1 Goal Contract、计划和预算属于不同状态

[Agent Loop 与状态机](/masterpiece-static-docs/05-模型接口与Agent内核/06-Agent-Loop与状态机.md)已经定义 Goal Contract、验收基线（Acceptance Baseline）和 Evidence Packet。进入 Multi-Agent 后，关键变化是这些对象必须跨 Parent 与 Child 保持同一版本。Parent 在 Fan-out 前冻结基线；计划可以随新证据产生 `plan_revision`，但每个 Child Envelope 都要携带同一 `baseline revision + digest`。Child 若收到不匹配的 Digest，应拒绝执行，而不是自行选择一个版本。

三类对象的可变性不同：

| 对象                  | 谁能提出变化                    | 谁能提交变化         | 规则                               |
| ------------------- | ------------------------- | -------------- | -------------------------------- |
| Acceptance Baseline | 用户或上层业务流程                 | Parent Harness | 一个 Attempt 内不可变；目标变化产生新 Revision |
| Plan                | Planner、Strategist Advice | Parent Harness | 可以修订路径，不能删除未满足 Criterion         |
| Evidence            | Worker                    | Artifact Store | 只追加新版本；已有 Artifact 不原地覆盖         |

Parent Budget 是所有 Child Budget 的硬上限。创建 Child 时先预留步骤、Token、费用、Deadline 和并发额度；Child 未使用的额度可以归还，已经消耗的高水位不能因 Context Compaction、Child Retry 或 Parent 恢复而清零。否则，一个表面有界的父任务会通过反复委派变成无界执行。

### 6.2 角色名称必须落到 Capability 与写权限

[Harness Engineering、架构模式与 Multi-Agent 边界](/masterpiece-static-docs/05-模型接口与Agent内核/07-架构模式与多Agent边界.md)已经说明不同职责不能共享权威状态写权限。具体到 Multi-Agent 拓扑，角色还必须成为可执行的能力信封（Capability Envelope），而不是人格设定：

| 角色             | 最小能力                                        | 可提交的结果                   | 禁止写入                                  |
| -------------- | ------------------------------------------- | ------------------------ | ------------------------------------- |
| Planner        | 读取 Goal、约束、可用能力和已有 Gap                      | Candidate Plan           | Acceptance Baseline、Domain State、完成终态 |
| Worker         | 仅本子任务需要的 Tool、数据和隔离工作区                      | Artifact、Evidence Packet | Goal 状态、其他 Child 状态、Verifier Verdict  |
| Strategist     | 读取基线、计划、Gap 历史和脱敏 Trace；可使用私有 Scratch Space | Strategy Advice          | 当前 Plan、生产数据、预算和权限                    |
| Verifier       | 读取冻结基线、Evidence Packet 和权威只读来源              | Verdict、Structured Gap   | 被验收 Artifact、计划、领域副作用                 |
| Parent Harness | 状态 Reducer、预算账本、Capability Gate             | 权威 Transition            | 绕过策略直接执行模型建议                          |

“私有”不仅指独立消息数组。Child 应拥有独立 Context Manifest、临时工作区、凭证和取消句柄；Verifier 不应继承 Worker 的可写工具，Strategist 也不能因为负责“提出新办法”而获得更高权限。权限沿委派链只能衰减；这里的交集必须同时作用于 Tool、Canonical Resource、Path、Network Origin、额度和期限，不能只比较能力名称：
...
四种职责不一定对应四个长期运行的 Agent。系统可以按风险使用确定性 Planner 或 Verifier，也可以让同一模型在隔离调用中承担不同职责；但不能让同一段未隔离的生成过程既创作证据，又修改标准并批准自己。
...
Worker 返回的 Artifact 只有进入 Evidence Packet 后，才具备可验收语义。Multi-Agent 场景至少还要记录 Producer 和输入版本：
...
type ChildEvidence = {
  ...
  criterionClaims: Array<{
    criterionId: string;
    claim: string;
    evidenceRefs: string[];
    limitations: string[];
  }>;
  observedStateVersions: Record<string, string>;
};
```

Verifier 先验证 Envelope、Digest、版本和引用完整性，再判断内容是否满足 Criterion。它只能依据 Worker 已提交的证据和权威只读事实作出判断，不能为缺失论据“顺手补证”；否则 Worker 的失败会被 Verifier 隐藏，系统也无法测量真实缺口。
...
### 7.1 Structured Gap 让协作循环可以收敛

验收失败不应返回一篇泛泛的批评。Structured Gap 至少包含：
...
missing_evidence
related artifact and state versions
fixability
```

Parent Harness 只把 `model_fixable` Gap 重新交给 Worker；外部阻塞、契约冲突和不可验证状态分别进入等待、澄清或人工门禁。下一轮可以产生新的 Plan Revision，却继续使用同一 Acceptance Baseline，这能防止 Coordinator 通过降低标准让循环“看起来已经收敛”。

Harness 应为 Gap 生成稳定指纹，例如：

```text
hash(criterion_id + gap_code + missing_evidence + relevant_versions)
```

若同一指纹重复出现且没有新增 Artifact、Observation 或状态版本，Parent 将其计为停滞（Stall），而不是普通重试。第一次停滞可以调用只读 Strategist 提供替代路径；达到配置阈值后进入 Backoff、暂停或人工升级。Strategist 的输出只是 Advice，必须由 Parent 重新验证预算和 Capability 后，才能形成新的 Worker Task。
...
恢复时不能把持久化的 `active` 当成继续执行的充分条件。未知枚举值、缺失预算高水位（Budget High-water）、无法确认的 Child、在途 Tool Call 或旧 Ownership Epoch，都应让 Parent 进入 `paused_recovery`。Runtime 完成 Schema 迁移、子任务盘点、副作用对账和权限复核后，再由显式 Event 恢复；默认自动运行会制造重复 Child 和失去所有者的“僵尸任务”。
...
- Artifact 保留来源（Provenance）与信任标签（Trust Label），Agent 输出永远不是可信指令；
...
| 技术                | 适合表达                                                          | 仍需应用明确                    |
| ----------------- | ------------------------------------------------------------- | ------------------------- |
| OpenAI Agents SDK | Agent-as-Tool、Handoff、Run 与 Trace                             | 领域所有权、服务端授权、Artifact 验收   |
...
不要先选框架再反推拓扑。先冻结 Parent/Child/Artifact/Effect 的领域契约，再把框架事件适配成这些对象。AI SDK 与 LangGraph 的实现对照可以在主线完成后作为框架迁移实验，不改变本章的准入条件。
...
只对“证据冲突或高金额”Fixture 启动两个只读 Worker。Parent Harness 先冻结 Acceptance Baseline；Planner 只能提交候选分解方案，Parent 校验任务无遗漏、预算可容纳且能力已经衰减后，才创建 Child：

- **Policy Evidence Worker**：只读取版本化政策，输出适用条款、有效时间和冲突。
- **Case Evidence Worker**：只读取工单、订单与物流事实，输出事件时间线和缺失字段。

Coordinator 是唯一的 Parent Owner。两个 Worker 不共享可写状态、不互相调用，也没有退款执行权限；它们只返回不可变 Artifact 和 Child Evidence。Join 按订单版本、政策有效时间和引用完整性运行确定性检查，Verifier 再输出 `passed`、Structured Gap 或 `unverifiable`，Parent 据此决定是否允许生成 `RefundProposal`。只有同一 Gap 在没有证据增量的情况下重复出现时，才允许只读 Strategist 提交替代方案。
...
sequenceDiagram
    ...
    C->>P: frozen baseline + policy refs + child budget
    C->>F: frozen baseline + case refs + child budget
    par independent work
      P-->>C: Policy Artifact + Child Evidence
    and
      F-->>C: Case Artifact + Child Evidence
    end
    C->>V: baseline + evidence packets + expected versions
    V-->>C: passed / structured gaps / unverifiable
    C-->>C: transition by reducer
```

故障注入至少包括：重复结果、迟到结果、相互冲突的证据、恶意 Artifact 内容、一个 Worker 超时、Parent cancel、政策版本在汇合前变化、Verifier 尝试补写证据、相同 Gap 无进展地重复，以及 Coordinator 在创建 Child 后崩溃恢复。恢复出的未知 Child 状态必须停在 `paused_recovery`，不能自动重新委派。
...
至少报告：任务成功率、错误完成率、证据覆盖率、重复工作率、契约违规率、Gap 修复率、停滞率、冲突识别率、P50/P95 延迟、总 Token、单位成功任务成本、Fan-out、取消收敛时间和各类失败占比。所有路径使用同一 Acceptance Baseline 和总预算；Multi-Agent 只有在目标切片上持续改善结果，并且恢复与权限测试通过时才保留，否则应删除这条路径。
...
- Planner 修改计划时可以顺便删除难以满足的验收项。
- Verifier 可以生成缺失证据，再批准自己补全后的结果。
- Context Compaction 或进程重启后，Parent 与 Child Budget 可以重新计算。
