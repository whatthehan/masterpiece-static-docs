...
> 版本与状态核验基准：2026-07-19。本章涉及 AI SDK 7、preview/experimental 状态和框架能力的结论均基于文末官方资料；实施时必须重新核对目标版本。框架版本变化不应进入领域契约。
...
Agentic UI State   Application Server / Canonical Event / Snapshot / Reducer
Agentic UI Runtime AG-UI / native transport / AI SDK UI
Generative UI      fixed tool-result component / A2UI Catalog
Embedded Agent UI  MCP Apps sandboxed iframe / AppBridge
...
| 真实互操作需求                                         | 候选协议  | 学习位置                                                                                               |
| ----------------------------------------------- | ----- | -------------------------------------------------------------------------------------------------- |
| Agent Backend 向产品 UI 发送运行、消息、Tool 与 State Event | AG-UI | 完成 Canonical Event 后实现 [AG-UI 前端事件适配](/masterpiece-static-docs/05-模型接口与Agent内核/10-AG-UI与前端事件适配.md) |
| Agent 生成由本地组件受控渲染的声明式界面                         | A2UI  | 完成 Agent UX 后实现至少一个澄清或证据收集 Surface                                                                 |
| 两个独立 Agent 系统发现能力并协作完成长任务                       | A2A   | 单 Agent Baseline、Identity 与可靠行动稳定后学习                                                               |
| Host 发现和调用外部 Tool、Resource 与 Prompt             | MCP   | Tool Contract 与 Authorization 之后学习                                                                 |

Agentic UI 是本书的核心主线，不是 Agent Runtime 完成后的界面包装。学习顺序是：先建立 Canonical Event、Public Snapshot、Reducer 与断线恢复，再实现 AG-UI Adapter 和受控的 Agent UX，最后用 A2UI 完成至少一个低风险的澄清或证据收集 Surface。Resolution Desk 必须用同一组 Fixture 对拍 Native Event Projection 与 AG-UI Adapter，证明协议替换不会改变领域状态；A2UI Surface 则必须经过固定 Catalog、Trusted Renderer 与 Action Gateway。

这条主线同时固定四个架构事实：AG-UI 位于 Product Edge；协议 Run 不等于业务 Outcome；共享 State 不是领域事实源；前端 Action 必须回到服务端重新认证、授权和校验。A2UI 只负责声明式 Surface，不接管领域 Canonical Event；退款 Preview、Approval 与 Commit 等高风险交互继续由固定原生组件和专用服务端 Command 承担。协议实现始终封装在 Adapter 内，因此完成必修实践并不等于把领域模型绑定到某个 SDK 版本。
...
flowchart TD
    ...
    A -->|是| U["完成 Agentic UI 核心主线<br/>Canonical Event · AG-UI · Agent UX · A2UI"]
    U --> C{"还需要哪类运行能力?"}
