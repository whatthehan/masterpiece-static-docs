...
Agentic UI 核心路径在此完成第一阶段。下一章进入 [Multi-Agent：协作、状态与验证](/masterpiece-static-docs/05-模型接口与Agent内核/11-Multi-Agent协作状态与验证.md)；随后通过 Context、Tool 与安全章节补齐知识、行动和威胁边界，再在第 08/05 章回到 [Agent UX 与可控交互](/masterpiece-static-docs/08-安全与治理/05-Agent-UX与可控交互.md)。
