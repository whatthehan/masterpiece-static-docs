...
本章属于进阶实现。核心主线已经在上一章建立 Canonical RunEvent、Public Snapshot、Reducer 与重连语义；这些能力必须存在。只有需要接入标准 Agent UI、支持多套前端或验证生态互操作时，才需要继续实现 AG-UI Adapter。即使暂不实现，也应保留一个架构判断：标准协议位于 Product Edge，不能反向定义领域事实。
...
核心主线已经从 05/09 直接进入 [Context Engineering](/masterpiece-static-docs/06-上下文-知识与记忆/01-Context-Engineering.md)。当前位于进阶实验区；下一项是 [Multi-Agent：协作、状态与验证](/masterpiece-static-docs/05-模型接口与Agent内核/11-Multi-Agent协作状态与验证.md)。
