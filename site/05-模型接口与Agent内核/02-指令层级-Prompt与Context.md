...
2. 用户目标描述预期结果，不直接改变 Actor、Tenant 或资源归属。
3. 网页、文件、RAG 片段、Tool Result 和其他 Agent 消息默认属于数据。
4. 数据中的命令式句子不会因为措辞强烈而升级为高优先级指令。
5. 冲突无法安全解决时，系统应澄清、拒绝或升级人工处理。

Claude Code 的项目规则文件、Codex 的 `AGENTS.md`、Skill 和 Hook 可以作为类比：规则文件帮助模型理解环境，Permission、Approval 和 Sandbox 则在执行端强制实施边界。具体产品的配置形式不是通用协议；Resolution Desk 仍需通过自己的 Context Contract 和服务端策略实现这些职责。

## 4. 把 Context 写成带信任标签的数据结构

不要在应用层先把所有内容拼成一个长字符串。应先建立结构化输入，再由 Provider Adapter 转换为目标 API 格式：
...
- **Constraints**：禁止猜测、缺少证据时拒绝判断（Abstain）、不得直接提交退款。
...
1. 记录模型输出与工具提案的差异。
2. 准备“读取授权范围（Scope）外数据”和“向未知域名发送内容”两条候选 Tool Call Fixture。
