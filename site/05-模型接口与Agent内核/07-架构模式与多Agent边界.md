...
- 划分 Planner、Worker、Strategist、Verifier 与 Harness 的权限和状态所有权。
...
acceptance_baseline_ref, plan_revision
```

新证据、权限变化、预算变化或工具失败，都可能触发重新规划（Replanning）。Plan Revision 可以改变路径和步骤，却不能修改它引用的验收基线。若用户调整目标，应先产生新的 Goal Contract Revision，再判断旧计划和旧证据是否仍然适用。Plan 描述意图，不能替代 Tool Gate、Approval 或完成验收。

## 9. Loop Engineering：Harness 持有目标与终止

Context Engineering 决定一次模型调用能看到什么；Harness Engineering 决定模型如何受控行动；Loop Engineering 则进一步处理长任务如何持续推进、证明完成并在无进展时停止。

Goal Loop 位于单次动作循环之上，但仍属于 Harness：

```mermaid
flowchart LR
    H["Harness<br/>Goal Contract · State · Budget"] --> P["Planner<br/>candidate plan"]
    P --> W["Worker<br/>bounded execution"]
    W --> E["Evidence Packet"]
    E --> V["Independent Verifier"]
    V -->|passed| H
    V -->|structured gaps| H
    H -->|authorized next work| W
    S["Strategist<br/>alternative approach"] -. advice .-> H
```

把四个角色写进 Prompt，并不会自动产生可靠分工。角色必须对应不同的能力边界和状态写权限：

| 参与者        | 可以做什么                              | 不能做什么                                 | 产物                       |
| ---------- | ---------------------------------- | ------------------------------------- | ------------------------ |
| Harness    | 冻结 Goal Contract、提交状态转移、划拨预算、授权和停止 | 把模型文本当作权威状态                           | Snapshot、Transition、最终终态 |
| Planner    | 读取目标与可用能力，提出或修订执行计划                | 修改 Acceptance Baseline、授予权限、提交领域写入    | Candidate Plan           |
| Worker     | 在衰减后的能力范围内执行有界任务                   | 自行宣布 Goal 完成、扩大工具权限                   | Artifact、Evidence Packet |
| Strategist | 在停滞或反复失败时分析 Gap，提出替代路径             | 直接改写当前 Plan、执行副作用、覆盖 Verifier Verdict | Strategy Advice          |
| Verifier   | 对冻结基线、Evidence Packet 和权威只读事实进行验收  | 生成被验收证据、修改基线、调用有副作用的工具                | Verdict、Structured Gaps  |

Planner、Worker、Strategist 和 Verifier 是职责，不必永久对应四个模型实例。低风险任务可以由同一模型在相互隔离的调用中承担不同职责；高风险任务则应增强独立性，例如分离 Context、工具权限、系统指令、数据来源或模型，并优先使用确定性 Verifier。无论采用哪种实现，Harness 都是唯一能够提交 Goal 状态、预算高水位和完成终态的参与者。

Strategist 也不应成为每轮必经节点。它适合在相同 Stall Fingerprint 重复出现、常规 Replanning 没有带来新证据时介入。其建议先作为不可执行的 Advice 保存，再由 Harness 选择是否产生新的 Plan Revision；这样，即使建议受到错误上下文影响，也不能绕过 Tool Gate 和 Acceptance Baseline。

### 9.1 四层 Loop 不应混用预算和身份

“循环”至少可能指四个不同层次：

| 层次                  | 一次迭代是什么                         | 状态所有者             | 典型终止条件                    |
| ------------------- | ------------------------------- | ----------------- | ------------------------- |
| Model / Tool Loop   | Proposal → Tool → Observation   | Run Reducer       | Final Item、等待输入、步骤预算      |
| Goal Loop           | Plan → Work → Evidence → Verify | Goal Orchestrator | Baseline 通过、阻塞、停滞、Goal 预算 |
| Durable Resume      | 从 Checkpoint 恢复同一 Attempt       | Runtime           | 原 Attempt 继续或进入恢复暂停       |
| Outer Orchestration | 创建并验收一个新 Attempt                | Task Orchestrator | 发布、人工门禁、总尝试预算             |

Context Compaction 不能重置 Run 或 Goal 的 Token 高水位；进程恢复不能创建新的自主身份；外层重试也不能复用旧 Attempt 的 Idempotency Key 冒充同一次提交。每层都应有自己的 ID、预算和终止 Event。

### 9.2 Durable Runtime 与 Outer Orchestration

在部分实践语境中，“Loop Engineering” 也专指更外层的自动工作闭环：系统从任务源发现工作，创建隔离环境，启动 Agent，独立验证，并在失败时创建下一次尝试。这个术语还没有统一边界；本书把这层明确称为 Outer Orchestration Loop，避免它与一次 Run 内的 Goal Loop 混淆。
...
同一 Run 的 Checkpoint / Resume 属于 Durable Runtime；验收失败后创建新 Attempt 属于 Outer Orchestration。恢复出的未知状态默认进入 `paused_recovery`，只有完成 Schema、预算、权限和在途副作用对账后才允许继续。两层必须分别设置预算、终止和幂等策略。
...
| Goal Orchestrator    | 目标、预算与完成状态由模型自报 | 收敛率、停滞率、恢复正确率  | 状态与迁移复杂度  | 任务退化为单次确定性调用  |
| Independent Verifier | Worker 自评偏宽     | 缺陷召回率、错误完成率    | Token 与延迟 | 确定性规则已完整覆盖    |
...
3. 对同一批退款工单冻结 Acceptance Baseline，比较固定 Workflow、单 Agent 和受限 Verifier；重复 Gap 只允许触发 Plan Revision，不能改写基线。本章不引入跨系统 Agent，风险复核的 A2A 路径留到 [A2A 与跨 Agent 协作协议](/masterpiece-static-docs/07-工具-协议与行动控制/05-A2A与跨Agent协作协议.md)。

### 验收证据

保存三组消融测试结果：Skill 超预算被拒绝、阻断型 Hook 失败时退款 Proposal 不进入后续执行路径、观察型 Hook 失败时既有结果不被回滚。固定 Workflow 与单 Agent 使用同一 Dataset、Acceptance Baseline 和总预算；Verifier 不能生成被验收证据，Strategist Advice 不能直接改变计划或状态。没有可测收益的额外角色不进入默认路径。
...
- 完整 Plan 一旦生成就不需要修改。
- Planner、Strategist 或 Verifier 可以直接更新 Goal 的权威状态。
- 只要调用另一个模型复核，Verifier 就天然独立。
- Multi-Agent 天然比单 Agent 更全面。

## 本章小结

Harness Engineering 把 Context、Loop、Tool、Policy、State、Sandbox、Extension 和 Trace 组织为可测试系统。Goal Contract、Evidence Packet、独立 Verifier 和停滞检测让“持续执行”收敛为可证明的终止，而 Planner、Worker、Strategist 只在 Harness 授权的边界内提供候选输入。Workflow、单 Agent、Outer Loop 与 Multi-Agent 解决的是不同控制问题，不是从旧到新的技术代际。下一章将据此评估[框架与 SDK 的学习优先级](/masterpiece-static-docs/05-模型接口与Agent内核/08-框架与SDK学习优先级.md)：先理解抽象替代了哪些 Harness 组件，再决定是否采用。
