...
浏览器调用普通 JSON API 时，收到 `200 OK` 后通常可以一次性解析完整响应。模型 API 的行为更像一个事件源：文本、推理对象（Reasoning Item）、Tool Call 参数、拒绝和用量信息可能分批到达，连接也可能在任意位置中断。若应用只把所有增量（Delta）拼成字符串，就无法判断工具参数是否完整，也无法可靠区分“用户看到了部分文本”和“本次运行已经完成”。
...
有状态调用（Stateful）与无状态调用（Stateless）只是 Provider State 的两种管理方式。无论采用哪一种，应用都必须保留自己的业务状态和版本。
...
应用应按 `event.type` 分发，而不是从文本前缀猜测类型；同时保留 Response ID、Item ID、Content Part ID 和 Call ID。一个简化的事件组装器（Assembler）可以这样设计：
...
Chat Completions Streaming 与 Responses Streaming 不能共用同一个“拼接增量”分支。前者主要返回带 `delta` 的增量数据块；后者返回带类型的 SSE 事件。迁移时应重新建立事件分发器（Event Dispatcher），而不是只替换 API 端点。

流式输出也会改变内容安全（Content Safety）边界：部分输出在终稿完整之前更难进行分类与审核；“完整生成后通过”的结果不能反向约束已经展示的增量片段。高安全场景应在展示前分段缓冲，并执行安全检查或内容审核；也可以改为完整输出后再展示。低延迟与展示前审查之间存在必须明确承担的成本权衡。
...
示例省略了不同 Output Item 的完整联合类型，不能直接复制成生产解析器。生产实现还应记录本次流所属的 `response.id`，并校验事件中 `item_id`、`output_index`、`content_index`、`call_id` 和 `sequence_number` 等字段在出现时的关联关系。Provider 的 `sequence_number` 用于解释本次 Response 内的顺序，不等于应用为持久化重放定义的 `RunEvent.seq`。
...
Adapter 升级 SDK 后，应先重放这些 Fixture。展示类未知事件可以记录并忽略；可能改变 Tool、状态或权限语义的未知事件应采用失败关闭（Fail Closed）策略，不能静默降级为普通文本。
...
- Provider 响应完成可能被错误映射为业务 Run 完成。
...
| 层                  | 负责                            | 不负责            |
| ------------------ | ----------------------------- | -------------- |
| Provider Adapter   | 解析厂商协议、闭合 Item、保留 Provider ID | 判断退款是否成功       |
| Canonical RunEvent | 表达稳定的应用语义                     | 暴露原始推理内容或未校验参数 |
| Transport Adapter  | 序号、重连、心跳和兼容                   | 定义业务状态         |
...
连接断开、内容截断或 Provider 将响应标记为 `incomplete` 时，应保留已接收内容用于诊断，但不能将其升级为可执行动作。用户已经看到一段文本，与 Runtime 已经提交结果，是两个不同事实。

## 8. 错误按发生层分类

| 层               | 示例                  | 主要处理位置                     |
| --------------- | ------------------- | -------------------------- |
| Transport       | DNS、TLS、连接中断        | HTTP 客户端 / Network Adapter |
| Protocol        | 未知必需事件、缺字段、关联 ID 错误 | Provider Adapter           |
| Provider        | 限流、服务错误、拒绝、截断       | Model Gateway / Runtime    |
| Model semantics | 错工具、错误参数、无依据结论      | 校验 / Policy / Eval         |
| Tool            | 超时、业务冲突、权限拒绝        | Tool Adapter / Executor    |
...
不连接真实模型，先为同一条退款资格判断准备类型化的事件 Fixture：

1. 文本由三个增量片段组成，并以完成事件正常关闭。
2. Tool Call 参数跨四个增量片段，在第三个片段后断流。
3. 同一个完成事件被重放两次。
4. Provider Response 已经完成，但业务 Run 仍在等待审批。
...
- 重复事件不会重复创建 Item；
- Provider 的完成事件不会把等待审批的 Run 标记为完成。
...
模型 API 返回的是带类型、生命周期和关联 ID 的事件，而不是一个最终字符串。模型提供方状态、应用状态和本轮 Context 分属不同层；Provider Event 也必须经过 Adapter 才能成为稳定的产品事件。下一章将用 [JSON Schema](/masterpiece-static-docs/05-模型接口与Agent内核/04-JSON-Schema基础.md) 为模型输出和工具参数建立运行时结构契约。
