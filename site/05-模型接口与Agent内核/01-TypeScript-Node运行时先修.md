# 01 · TypeScript + Node：构建 Agent Runtime 前需要补齐的能力
...
实现一个只允许 `get_order` 与 `get_refund_policy` 的 Runtime：

1. 模型输出必须经过运行时校验。
2. 两个只读工具最多并发 2 个。
3. 所有调用共享一个 deadline 与 `AbortSignal`。
4. 取消后不能创建新的 Tool Call。
5. Trace 能区分模型完成、工具完成、Run 完成和失败。

验收时分别注入半截 JSON、工具超时、重复 event 和主线程阻塞。测试的目标不是证明正常路径能跑通，而是证明边界状态不会被误判。
