...
完成全书后，应当能够回答并实现六件事：模型为什么会产生当前结果，Agent 如何选择和执行下一步，人类如何理解并控制任务，失败后怎样恢复，用什么证据证明任务真的完成，以及生产证据如何经过独立评测与发布门禁成为下一版系统。
...
生产 Trace、用户修订和权威 Outcome 会继续产生改进信号，但信号不是线上修改指令。它们先被整理为 Finding、Eval Target 和 Change Proposal，再形成候选 Behavior Bundle；候选只有通过独立 Holdout、安全门禁、审批和受控发布，才能影响后续 Run。这条 Controlled Improvement（受控改进）闭环属于工程证据线，不是 Agent Runtime 获得“自行改写自己”的权限。
...
| Improvement | 候选 Prompt / Model           | Finding、Change Proposal、Candidate Bundle | 改进证据与版本状态             | Holdout、安全评测、Canary、Rollback     |
...
4. 完成第 05 部分 01–10：先建立 Model Adapter、有界 Loop 与 State，再用 Application Server 和 AG-UI 建立 UI 运行时边界；
5. 阅读第 05 部分 11–12，先掌握 Multi-Agent 的状态责任和 Framework 对照方法；具体协作实验与 Durable Tier 分别在第 07、09 部分完成后回访；
6. 完成第 06 部分和第 07 部分，依次加入 Context、Knowledge、Memory、Tool、MCP、受控行动、A2A 与 Skills；其中 Agentic RAG / GraphRAG 是进阶检索实验；
7. 按第 08 部分 01–07 的顺序推进：先建立 Threat Model、Injection 防线、最小权限和纵深防御，再完成 Agent UX、A2UI 与 Red Team；
8. 用第 09 部分补齐恢复、可观测、发布门禁与受控改进闭环，并回访第 05/12 的 Durable Tier；
9. 第 10 部分是可跳过的 Rust 与跨语言边界专题；只有基准证据表明 TypeScript + Node.js 已成为瓶颈时再进入；
10. 在第 11 部分按“综合心智模型 → 自测 → 参考答案 → 八周构建路径 → Resolution Desk 总装”完成闭环。

Agentic UI 核心路径分成两个阶段。第 05/09–10 先回答运行状态如何可靠抵达 UI；第 08/05–06 再回答用户如何控制任务，以及 Agent 如何在受限 Catalog 中描述界面。中间的 Context、Tool 与安全章节不是插入内容，而是 Agent UX 和 A2UI 成立所需的知识、权限与威胁边界。

Agentic RAG / GraphRAG、Framework 对照、Skills、Rust、资料索引、能力索引和场景迁移属于专题与附录，不影响核心项目完成。
