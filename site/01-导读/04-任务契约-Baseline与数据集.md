...
这一步不包含 Agent Runtime，而是先固定任务、边界和比较方法，为后续每一层实现提供共同尺度。起点只需要 Task Contract、非 Agent Baseline、3 个 Anchor Case 和一个最小 Grader；Dataset 会在后续章节随能力与故障逐步增长，不需要在开始时一次写完。
...
## 5. Dataset：从 3 个 Anchor Case 开始增长

项目起点只建立 3 个 Anchor Case：正常退款、信息不足和跨租户请求。它们分别证明系统能够完成、能够克制、能够拒绝。此时尚未实现 Model Adapter、Tool Loop 或故障恢复，提前编写 30–50 个详细任务只会让样本依赖尚不存在的接口。

后续每引入一项能力，就把相应边界加入 Dataset：学习采样时增加多 Trial Case；加入 Retrieval 时增加权限、时效与冲突 Case；开放 Command 时增加 Approval、重复请求和 ACK 丢失 Case；进入安全章节后再增加 Prompt Injection。全书总装时形成 30–50 个高信息量 Task，而不是在第一章凑齐数量。
...
- 一条 Dataset 增长规则：每学到一种新能力或发现一种新失败，就增加对应 Regression Case。
...
这一步把“想做一个 Agent”转换为可比较的工程问题：Task Contract 定义边界，Baseline 证明复杂度是否必要，3 个 Anchor Case 固定最早的完成、澄清和拒绝行为，Grader 检查真实 Outcome。下一章将说明这些材料如何随 Resolution Desk 一起逐步生长。
