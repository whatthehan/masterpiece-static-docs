...
### 里程碑 4 · 第一条单 Agent 纵向切片与 UI 事件面

**进入时**：任务与评测已定义，还没有 Agent Runtime。

**增加**：Provider 官方 SDK Adapter、Streaming Event、Structured Outputs、3–5 个只读 Tool、有界 Loop、Budget、Cancel 和内部 Run State；随后加入 Application Server、Canonical Event、Public Snapshot 与 AG-UI Adapter。

**三层增量**：LLM 开始产生结构化 Item；Agent 层完成最小 Harness；UI 不再直接消费 Provider Event，而是从可重放的应用状态观察 Run 和 Tool Timeline。

**验收**：半个 Tool Call 永不执行；取消后不产生新动作；每次失败能归因到 Model、Protocol、Validation、Tool 或 Runtime；页面刷新、事件重复和序号缺口不会制造第二份业务事实。

对应阅读：[模型接口与 Agent 内核 01–10](/masterpiece-static-docs/05-模型接口与Agent内核/01-TypeScript-Node运行时边界.md)。随后第一次阅读 05/11–12，先建立 Multi-Agent 和 Framework 判断框架；具体实验在依赖补齐后回访。
...
### 里程碑 8 · 将安全边界投影为 Agentic UI

**进入时**：第 05/09–10 已经建立 Application Server、Canonical Event 与 AG-UI Adapter；Knowledge、Tool 与受控行动随后补齐，但产品界面尚未系统表达权限、未知效果和人类控制。

**增加**：先用 Threat Model、Prompt Injection 防线、最小权限与纵深防御固定 UI 的安全前置，再加入 Stop / Interrupt / Resume / Handoff、可信 Approval UI，以及受 Catalog 约束的 A2UI Renderer 与服务端 Action Gateway。
...
对应阅读：按第 08 部分 01–06 的顺序完成安全前置、[Agent UX 与可控交互](/masterpiece-static-docs/08-安全与治理/05-Agent-UX与可控交互.md)和[A2UI 与声明式生成界面](/masterpiece-static-docs/08-安全与治理/06-A2UI与声明式生成界面.md)。

### 里程碑 9 · 系统性安全门禁

**进入时**：三层功能已经闭合，UI 的 Trusted Renderer、Action Gateway、Approval 与 Handoff 边界已经明确，但尚未经过完整攻击链验证。

**增加**：把上一里程碑建立的 Threat Model、数据流策略、最小权限和纵深防御组合成攻击链，覆盖恶意 Tool Description、跨租户读取、权限升级、数据外泄、A2UI 注入和跨 Agent Artifact 污染。
...
对应阅读：[Agent 安全评测与 Red Team](/masterpiece-static-docs/08-安全与治理/07-Agent安全评测与Red-Team.md)；第 08 部分 01–06 已在上一里程碑按目录顺序完成。
...
前两篇建立状态与协议边界，后两篇建立人类控制与声明式界面边界。中间先完成知识、行动和安全约束，避免在边界不完整时实现动态 UI。Resolution Desk 的主线验收因此包含：
...
跳过第 10 部分的 Rust 可选专题不会缺少任何核心产品能力。进入该专题时，验收标准仍是与 TypeScript 版本共享 Contract、Fixture、Trace 与故障语义，而不是迁移代码量。
