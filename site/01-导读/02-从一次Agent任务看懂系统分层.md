...
生产证据还会暴露重复失败和新的改进机会，但 Trace、用户反馈或一次优秀结果都不能直接改写在线 Prompt、Tool、Policy 或代码。它们只能先形成 Finding 与 Eval Target，再由隔离实验生成候选 Behavior Bundle，经独立评测、审批和发布门禁影响后续 Run。这样，“持续变好”仍然属于可验证的软件交付，而不是运行中的 Agent 获得修改自身规则的权限。
