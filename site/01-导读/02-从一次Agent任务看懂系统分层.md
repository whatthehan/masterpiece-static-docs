...
一项 Coding Agent 任务在界面上表现为连续对话，背后却由职责不同的组件协作完成。只有拆开这些职责，才能判断问题应当由 Prompt、Context、Tool、Runtime 还是业务策略解决。
...
模型（Model）接收 Token 序列，生成文本、结构化数据或 Tool Call。预训练和后训练决定它擅长哪些模式；当前 Context 决定它在本次调用中能够看到哪些信息。
...
Claude Code 或 Codex 按需搜索文件，就是一种 Context 选择。若把整个仓库、全部历史和所有工具定义一次性放入请求，相关信号会被噪声稀释，成本和攻击面也会同步扩大。
...
Agent 运行环境（Agent Harness）把 Model、Context Builder、Runtime、Tools、Sandbox、Permission、Hooks、State、Compaction 和 Trace 组合在一起。Coding Agent 能够浏览代码库、执行命令，并在测试命令返回非零退出码后继续定位，主要依靠这套运行环境。
...
- 持续集成（Continuous Integration，CI）的最终结果和 API 兼容性检查才是 Outcome 证据。
...
先观察 Coding Agent 中“Context → Candidate → Tool → Observation → Outcome”的结构，再用同一张表拆解任务 `case_refund_clear`：
